...
GOAL
Ship Heirloom desktop stamp 0.5.1 from GitHub onto this Emergent app (preview first, then promote to production).

SOURCE OF TRUTH
- GitHub: https://github.com/Unbound-Infotech-Corporation/Heirloomunbound
- Sync EXACTLY: tag v0.5.1
- Zip if git sync is broken: https://github.com/Unbound-Infotech-Corporation/Heirloomunbound/archive/refs/tags/v0.5.1.zip
- Operator notes in-repo after sync: EMERGENT_HANDOFF.md and EMERGENT_PASTE_THIS.md

DO THIS NOW
1) Sync / pull / replace the Emergent workspace with GitHub tag v0.5.1. Prefer a full tree sync over cherry-picking files. If your Git tool fails, unpack the tag zip over the app tree.
...
3) Confirm these files exist after sync:
   ...
   - backend/companion_desktop/heirloom/__init__.py containing __version__ = "0.5.1"
...
6) Verify:
   ...
   - GET /api/build → desktop_version MUST be "0.5.1"
...
9) Reply with: synced git commit SHA (must be the v0.5.1 tag tip), the full /api/build JSON, whether production promote succeeded, and any file that failed to sync.

If sync tools are broken, tell me the exact Emergent UI path to connect GitHub tag v0.5.1 and redeploy, then stop. Do not partially apply a half-update.
```
...
2. Confirm `"desktop_version":"0.5.1"`
3. In Emergent UI → **Deploy / Promote** to production
4. Open prod: https://voice-clone-hub-20.emergent.host/api/build
5. Confirm `"desktop_version":"0.5.1"` again

If preview is still old after they “synced”, they did not pull tag **v0.5.1**. Paste the block again and demand the tag tip SHA in their reply.

## Links

- Paste file on GitHub: https://github.com/Unbound-Infotech-Corporation/Heirloomunbound/blob/v0.5.1/EMERGENT_PASTE_THIS.md
- Tag zip: https://github.com/Unbound-Infotech-Corporation/Heirloomunbound/archive/refs/tags/v0.5.1.zip
