...
## Phase 43 — Feb 2026 (Mobile PWA + Twilio Voice SDK WebRTC)
User asked for a mobile app to make talking to the twin easier. Choice: **PWA install-to-home** (no separate native binary), targeting iOS + Android via one codebase.

- ✅ **PWA shell** — `/app/frontend/public/manifest.json` (name, start_url=/m, standalone, theme, icons, shortcuts), `/sw.js` service worker (network-first for `/api`, cache-first for static, offline fallback to cached `/m`), Apple/Android meta tags in `index.html` (`apple-mobile-web-app-capable`, `apple-touch-icon`, `viewport-fit=cover`), auto-generated 192/512 icons with an `H` monogram in accent amber. SW only registers in production builds.
- ✅ **`/m` mobile shell** — `MobileShell.jsx` with bottom tab bar (Call/Capture/History), safe-area insets for notched devices, `beforeinstallprompt` capture so Android/desktop users see an "Install to home screen" button in-app.
- ✅ **`/m/call` — dual-path calling** — WebRTC in-app dialer via `@twilio/voice-sdk` 2.x + PSTN `tel:` fallback + graceful blocked states (no Twilio config → link to /phone setup; no WebRTC keys → link to setup). Live call state machine (idle→connecting→ringing→in-call→ended) with mute + hangup + duration counter.
- ✅ **`/m/capture`** — voice memo via MediaRecorder (WebM/Opus, one-tap record→stop→upload to `/entries/voice`) and rear-camera photo capture via `<input capture="environment">` (upload to `/photos`).
- ✅ **`/m/history`** — timeline blending recent Twilio calls + text/voice entries + photo thumbnails.
- ✅ **Backend Twilio config extended** — `TwilioConfig` gained optional `api_key_sid` + `api_key_secret` + `twiml_app_sid` fields (all separate from the existing Account SID + Auth Token). `ConfigStatus` gained a `webrtc_configured` boolean.
- ✅ **`POST /api/twilio/voice/token`** — mints a JWT via `twilio.jwt.access_token.AccessToken` with `VoiceGrant` (incoming allowed + outgoing pointing at the user's TwiML App). Returns 400 with clear setup instructions when the WebRTC fields are missing. Verified JWT payload: sub=account_sid, iss=api_key_sid, identity=`twin-<user_id>`, TTL=3600.
- ✅ **Tested** — iteration_43.json: 12/12 backend + 7/7 frontend flows green. JWT payload correctness, all mobile tabs, PWA assets, empty states, auth-guard on `/m`, no regressions. WebRTC negotiation itself fails with the fake seeded creds (`AccessTokenInvalid`) — expected in preview pod.

## Phase 42 — Feb 2026 (Template Comparison + Projection History + Copy Tooltip + Template Editor)
- ✅ **Template Comparison Table** — new `GET /api/routing/templates/preview?template_id=...` returns `{current_routes, new_routes, diff:[{task, task_label, from, to}]}`. Frontend now opens a modal on template click showing the diff table before Apply confirmation. Empty diff → "No changes" notice + disabled confirm. Cancel/X/backdrop-click all close without applying.
- ✅ **Projection History Chart** — new `snapshot_projections(user_id)` upserts `projection_history` keyed on (user_id, provider, day). Background 6h loop registered as `app.state.projection_task`. `GET /api/routing/usage/projection/history?days=14` returns per-day series. Frontend renders a small "trend" sparkline below the daily one on cards that have snapshots.
- ✅ **Copy Sparkline Tooltip** — `<Sparkline>` SVG now has `onClick` → `navigator.clipboard.writeText("YYYY-MM-DD, $X.XXXX")` + success toast. Cursor changes to pointer on hover. Tooltip shows a small Copy icon so users know it's clickable.
- ✅ **Template Editor** — `POST /api/routing/templates/save` creates a `user_<hex>` template scoped to the caller; `DELETE /api/routing/templates/{template_id}` removes it (only `user_` prefix, only for the owning user). `GET /templates` merges built-ins + user's own with a `kind` field. Frontend has "Save current as template" button (opens name/blurb modal) and each custom card shows a "yours" badge + trash icon.
- ✅ **Indexes** — added `user_templates (template_id) unique`, `user_templates (user_id, created_at)`, `projection_history (user_id, provider, day) unique`, `projection_history (user_id, day desc)`.
- ✅ **Tested** — iteration_42.json: **26/26 backend + full frontend flow verified.** Cross-user scoping proven (UA vs UB), preview 404s correctly on foreign templates, delete of built-in returns 400, snapshot idempotency confirmed. One stylistic non-issue: empty-label save button is disabled instead of toasting an error (better UX per my judgment).

## Phase 41 — Feb 2026 (Provider Templates + Cost Projection + Sparkline Tooltip + Retry-Cap Fix)
- ✅ **Provider Templates** — `PRESETS` catalog in `services/llm_router.py` with 5 one-click presets (cheapest, quality_first, balanced, all_emergent, local_first). New endpoints `GET /api/routing/templates` + `POST /api/routing/templates/apply` — only touches `task_routes`, leaves keys/enabled/budgets untouched, returns `warnings` for any preset-required provider that's disabled or unkeyed. Frontend "One-click presets" section renders 5 tappable cards with icon, blurb and required-provider chips.
- ✅ **Cost Budget Prediction** — `services/llm_router.month_end_projections()` sums this month's usage per provider and linear-extrapolates `mtd / days_elapsed * days_in_month`. `GET /api/routing/usage/projection` returns `{provider_id: {mtd_usd, projected_month_end_usd, days_elapsed, days_in_month}}`. Frontend renders a `PROJ · $X.XXXX` chip below each sparkline; when projected month-end exceeds the provider's cap, chip turns amber.
- ✅ **Sparkline Tooltip** — `<Sparkline>` component now tracks a `hoverIdx` state driven by `onMouseMove`. Hover reveals a floating tooltip `YYYY-MM-DD · $X.XXXX`, a dashed vertical guide line, and a highlighted point at the hovered index. Mouse leave hides everything.
- ✅ **Rotation Alert Retry Cap** — `_write_health` in `services/provider_health.py` now uses a `rotation_alert_attempts` counter capped at 3. Each failed Resend delivery bumps the counter; at 3 the `log.warning('gave up after 3 failed sends')` fires and further green→red attempts no-op. Both `rotation_alert_sent` AND `rotation_alert_attempts` reset to zero on the next green probe.
- 🐛 **Bug caught + fixed by iteration_40 → 41 loop** — initial `_write_health` projection missed `rotation_alert_attempts: 1` in the Mongo find_one — counter always re-read as 0 and the 3-strike cap never engaged. Fix landed post-report, iteration_41 confirmed 13/13 backend + retry-cap semantic verified end-to-end (accumulate 1→2→3, no-op at 3, reset on green, next cycle starts fresh 3-attempt window).

## Phase 40 — Feb 2026 (Regression Tests + Rotation Guide + Rotation Alerts + Cost Chart)
- ✅ **Restoration regression suite** — `/app/backend/tests/test_restoration_regression.py` — 18 tests across four buckets: state machine (blocked/queued/foreign-photo/invalid-kind), device-auth boundary (session-token rejected, foreign device rejected, photo-file scoped), idempotency (double result upload returns same photo_id + no duplicate; double fail is a no-op), magic-byte gate (HTML.png/zero-byte rejected 415; PNG/JPEG magic accepted 200). **18/18 pass locally + verified by testing_agent.**
- ✅ **Secrets rotation playbook** — `/app/memory/SECRETS_ROTATION.md` — operator checklist for rotating Stripe, Resend, ElevenLabs, D-ID, Fal.ai, GitHub OAuth, Spotify OAuth (least→most disruptive), verification curl commands, and ongoing hygiene guidance. I can't rotate live vendor keys — this document is what the owner runs through.
- ✅ **Rotation alerts** — `services/provider_health.py::_write_health` now detects the green→red transition on a per-provider row and fires a Resend email via `email_service.send_provider_rotation_email`. The `rotation_alert_sent` flag on `provider_health` is the idempotency guard; it flips to True on send and resets to False on the next green probe, so a red → green → red cycle re-fires exactly once. Verified end-to-end by testing_agent.
- ✅ **Router cost chart** — new `services/llm_router.daily_spend_series()` + `GET /api/routing/usage/daily?days=N` (clamped [1..90]) returns `{days: [YYYY-MM-DD, ...], series: {provider: [floats]}}`. Frontend Routing page has a new `<Sparkline>` SVG component (no chart lib) that plots per-provider 30-day spend with a highlight dot on the last value and `30d · $X.XXXX` caption below. Renders only for providers with usage; the other 6 stay clean.
- 🐛 **Edge-case fix from testing_agent report** — `daily_spend_series` used `int(days or 30)` which collapsed `?days=0` to 30 instead of 1. Now: `int(days if days is not None else 30)` before the `max(1, min(_, 90))` clamp. Verified: `days=0 → 1`, `days=1000 → 90`, `days=30 → 30`.
